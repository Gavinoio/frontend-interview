...
::: details 2. Cache-Control 的 no-cache 和 no-store 区别？
```javascript
// no-cache
// - 可以缓存，但使用前必须验证
// - 每次都发送请求到服务器
...
::: details 4. ETag 和 Last-Modified 同时存在时的验证顺序？
```javascript
// 1. 服务器优先验证 ETag
